...
async function run() {
  try {
    ...
    app.get("/toys/:limit", async (req, res) => {
      let toy = toys.find();
      let result = await toy.limit(parseInt(req.params.limit)).toArray();
      res.send(result);
    });
    ...
    app.get("/mytoys/:email", async (req, res) => {
      let mytoysCursor = toys.find({ selleremail: req.params.email });
      let result = await mytoysCursor.toArray();
      res.send(result);
